...
define('PATH_site', str_replace("/modules/budget/bin", "/", dirname(PATH_thisScript)));
...
switch ($action) {
    case "open" :
        ...
        break;
    case "update" :
        $myInput = array(name => $name, type => $type, amount => $amount);
        $result = $myData->update($myInput, "id_profit=" . $id, $table);
        break;
    case "insert" :
        $myInput = array(name => $name, type => $type, amount => $amount);
        ...
        break;
    case "delete" :
        $result = $myData->delete("id_profit=" . $id, $table);
        break;
    default :
        $xml = "no action";
}
...
if ($result == "1") { // check for the result, and read the xml.
    $json = $myData->select("", $table, "all", "id_profit ASC", "", "");
} else {
    $json = '{"result":"error"}';
}
