...
var urlJson = "modules/budget/bin/profit.json.php"; // url for the json php file
var edit_id = 0;
...
function setBottonEvent(){
    ...
    $("#savebotton").bind("click", function(){
        ...
        // edit function
        editCall(edit_id, newname, newtype, newamount);
    ...
    if (data.result != "error"){
        ...
        if (totalRow > 0){            
            $.each(data.result, function(i, field){
                ...
                appendOpenTable(id, name, type, amount, creation_date);
            });
        }
...
// put data on table
function appendOpenTable(id, name, type, amount, creation_date ){
...
// edit function
function editCall(id, name, type, amount){
    // ajax call to edit and insert base on action
    var action = "update";
    if (id == 0){
        action = "insert"
    }
    getDatajson(urlJson+"?action="+action+"&id="+id+"&name="+name+"&type="+type+"&amount="+amount, "handlerEditCall");
}
